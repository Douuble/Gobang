...

{
...

    {
...

        {
            ...
            case '1': strcpy(p, "●"); j++; break;//j++可省
            case '2':  strcpy(p, "○"); j++; break;//j++可
            ...
            case 'i':  strcpy(p,"━"); break;//上下的横线更粗
            case 'j':  strcpy(p,"—"); j++; break;//中间的横线更细//j++;由于输出"—"时占两个字符，j++可以避开输出下一个的空格
            case 'k':  strcpy(p,"┃"); break;//左右的竖线更粗
            case 'l': strcpy(p,"│"); break;//中间的竖线更粗
            case 'm': strcpy(p,"┼"); break;
            case ' ': strcpy(p," "); break;
            default: strcpy(p,&buffer[i][j]);
